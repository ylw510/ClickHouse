...
TEST_F(MetadataPlainRewritableDiskTest, RecreateFileAfterUnlinkInSameTransaction)
{
    thread_local_rng.seed(42);

    auto metadata = getMetadataStorage("RecreateFileAfterUnlinkInSameTransaction");
    auto object_storage = getObjectStorage("RecreateFileAfterUnlinkInSameTransaction");
    ...
    /// The blob at the default location is removed and created again under the same key, then shared.
    {
        auto tx = metadata->createTransaction();
        tx->unlinkFile("A/f1", /*if_exists=*/false, /*should_remove_objects=*/true);
        const auto key = tx->generateObjectKeyForPath("A/f1").serialize();
        EXPECT_TRUE(key.ends_with("/f1"));
        size_t size = writeObject(object_storage, key, "v2");
    ...
    EXPECT_EQ(readObject(object_storage, metadata->getStorageObjects("B/f1").front().remote_path), "v2");

    metadata = restartMetadataStorage("RecreateFileAfterUnlinkInSameTransaction");
    EXPECT_FALSE(metadata->existsFile("A/f1"));
    EXPECT_EQ(readObject(object_storage, metadata->getStorageObjects("B/f1").front().remote_path), "v2");
}
