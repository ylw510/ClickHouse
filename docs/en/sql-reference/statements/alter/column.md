...
If the column is not trivially convertible, you can combine `MODIFY COLUMN` with [`UPDATE`](/sql-reference/statements/alter/update.md) in the same `ALTER` so the new values are computed from an expression of the previous value (and optionally other columns). Metadata is updated once and a single mutation rewrites the data:
...
The `UPDATE` expression is evaluated against the column's previous type; its result is stored as the new type. Rows not matched by `WHERE` still require a valid cast from the old type to the new type.
