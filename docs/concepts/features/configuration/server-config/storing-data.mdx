...
supports `INSERT` operations, `ALTER` operations, and
[mutations](/reference/statements/alter/index#mutations).
Replication of tables is not supported.

Mutations and `ALTER` operations rely on hard links between files. A hard link
makes the metadata of a directory list its files explicitly, together with the
keys of the objects storing them. Servers older than the version that introduced
hard links cannot read such directories, while directories that never had hard
links keep the previous layout and remain readable by older servers.
...
for the table. This could be useful, for example, for system tables.
