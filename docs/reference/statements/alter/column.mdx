...
If the column is not trivially convertible, you can combine `MODIFY COLUMN` with [`UPDATE`](/reference/statements/alter/update) in the same `ALTER` so the new values are computed from an expression of the previous value (and optionally other columns). Metadata is updated once and a single mutation rewrites the data:
